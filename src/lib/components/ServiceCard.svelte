<script lang="ts">
	...
	let {
		...
		qualityYears = 2,
		imageSrc = '',
		stockImageSrc = 'https://images.unsplash.com/photo-1589939705384-5185137a7f0f?auto=format&fit=crop&q=80&w=600',
		class: className = ''
	} = $props<{
		title?: string;
		themeColor?: string;
...
>
	...
	>
		...
		{#if qualityYears && qualityYears !== 0}
			<!-- Quality Strip -->
	...
	<!-- Mobile Table (Stock Image) / Desktop Left Column -->
	<div
		class="flex w-full flex-col border-b lg:order-1 lg:w-[30%] lg:border-r lg:border-b-0"
		style="border-color: var(--tc);"
	>
		<div
		...
		<div class="relative min-h-[15rem] w-full flex-1 overflow-hidden bg-gray-100 lg:min-h-0">
			<img
				src={stockImageSrc}
				alt="Servicio {title}"
	...
	<div class="relative flex flex-1 flex-col justify-between bg-white p-4 lg:order-2 lg:px-6">
		...
		<div
			class="relative flex h-full flex-col rounded-2xl border-[1.5px] p-5 lg:h-auto"
			style="border-color: var(--tc);"
		>
			<h3
			...
			<p class="text-[13px] leading-relaxed font-medium whitespace-pre-line text-gray-700">
				{description}
			</p>
		</div>
	</div>
